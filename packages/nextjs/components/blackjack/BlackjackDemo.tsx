...
export const BlackjackDemo = () => {
  ...
  const [dealerScore, setDealerScore] = useState<number | undefined | "?">(0);
      ...
      logs.map(log => {
        ...
        if (address == player) {
          ...
          setNewPlayerHand(prevState => {
            ...
            const mutablePlayerHand: number[][] | undefined = playerHand
      ...
      logs.map(log => {
        ...
        if (address == player) {
          ...
          setNewDealerHand(prevState => {
            ...
            const mutablePlayerHand: number[][] | undefined = dealerHand
              ? dealerHand.map(card => [...card])
              : undefined;

            if (mutablePlayerHand) {
              prevPlayerHand.push(...mutablePlayerHand);
            }
              ...
              <div className="flex flex-col gap-6 items-center">
                <div className="col-span-1 lg:col-span-2 flex flex-col gap-6">
                  <div className="flex items-stretch justify-start">
                    {newDealerHand?.length == 0 && <SkeletonCard />}
                    {newDealerHand && newDealerHand.map((data, index) => <Card data={data} key={index} />)}
                    {isActive && <UnknownCard />}
                  </div>
                </div>
                <div
                  className={`flex flex-col items-center rounded-3xl px-6 lg:px-8 py-4 ${
                    playerScore == 0 ? "opacity-10 bg-blue-400 bg-opacity-20 text-white text-opacity-0" : ""
                  }`}
                >
                ...
                <div className="col-span-1 lg:col-span-2 flex flex-col gap-6">
                  <div className="flex items-stretch justify-start">
                    ...
                    {newPlayerHand && newPlayerHand.map((data: any, index) => <Card data={data} key={index} />)}
                  </div>
                </div>
              </div>
            </div>
